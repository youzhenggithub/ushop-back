<template>
  <el-dialog @close="reset" :title="info.isAdd?'新增':'编辑'" :visible.sync="info.isShow">
    <!-- 分割线 -->
    <el-form :model="formInfo" label-width="120px" ref="formInfo" :rules="rules">
      
      <el-form-item label="属性名" prop="specs" placeholder="请输入属性名">
        <el-input v-model="formInfo.specsname" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="属性值" prop="attrs">
        <div class="box" v-for="(item,index) in list" :key='index'>
          <el-input type="text" v-model="list[index]" autocomplete="off" placeholder="请输入属性"></el-input>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        specsname: [{ required: true, message: "请输入账号", trigger: "blur" }],
